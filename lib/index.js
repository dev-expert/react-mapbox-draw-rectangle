...
class DrawControl extends React.Component {
    ...
    render() {
        return React.createElement("div", { className: 'custom-top-right' },
            React.createElement("div", { className: 'custom-group' }, Object.keys(this.props.modesConfig).map((key, index) => React.createElement("button", { key: index, className: "draw", onClick: () => {
                    if (this.draw)
                        this.draw.changeMode(key, this.props.modesConfig[key]);
                }, title: this.props.modesConfig[key].title }, " "))));
    }
}
